...
from cfgs.env import push_t_lang
...
export = d(
    ...
    # utils=utils,
    env_spec=GroupField('env_train', push_t_lang.export.cls.get_default_env_spec_params),
    env_train=push_t_lang.export,
    ...
    model=bc_rnn.export & d(
        goal_names=[],
        state_names=F('use_keypoint', lambda kp: ['state', 'lang_embed', 'keypoint'] if kp else ['state']),
        device=F('device'),
        action_decoder=d(
            hidden_size=400,
            use_tanh_out=False,
